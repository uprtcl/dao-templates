...
module.exports = class TemplateDeployer {
  ...
  async _fetchOrDeployMiniMeFactory() {
    const MiniMeTokenRaiserFactory = this.artifacts.require(
    ...
    if (this.options.miniMeFactory) {
      ...
      this.miniMeFactory = MiniMeTokenRaiserFactory.at(
        this.options.miniMeFactory,
      )
    } else {
      this.miniMeFactory = await MiniMeTokenRaiserFactory.new()
  ...
  async _registerPackage(name, instance) {
    if (this.options.register) {
      this.log(
        `Registering package for ${instance.constructor.contractName} as "${name}.aragonpm.eth"`,
      )
